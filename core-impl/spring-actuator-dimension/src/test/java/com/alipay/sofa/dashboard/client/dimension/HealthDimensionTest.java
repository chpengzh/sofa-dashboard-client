...
public class HealthDimensionTest {
    ...
    @Autowired
    private ActuatorHealthDimension dimension;

    @Test
    public void basicInvokeTest() {
        Assert.assertEquals(dimension.getName(), DimensionName.HEALTH);
        Assert.assertEquals(dimension.getType(), HealthDescriptor.class);

        HealthDescriptor descriptor = dimension.currentValue();
