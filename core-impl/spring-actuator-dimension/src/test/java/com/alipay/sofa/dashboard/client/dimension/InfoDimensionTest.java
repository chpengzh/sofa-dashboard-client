...
@ActiveProfiles("test")
public class InfoDimensionTest {

    private static final Logger   LOGGER = LoggerFactory.getLogger(InfoDimensionTest.class);

    @Autowired
    private ActuatorInfoDimension dimension;

    @Test
    public void basicInvokeTest() {
        Assert.assertEquals(dimension.getName(), DimensionName.INFO);
        Assert.assertEquals(dimension.getType(), InfoDescriptor.class);

        InfoDescriptor descriptor = dimension.currentValue();
